...
def run_clocks(clocks: List[Clock], duration: float, dt: float) -> Dict[str, np.ndarray]:
    ...
    Returns a dict with keys: 'time', 'clock_0', 'clock_1', ...
    """
...
def compare_clocks(time_s: List[float] | np.ndarray,
                   t_a_s: List[float] | np.ndarray,
                   t_b_s: List[float] | np.ndarray) -> Dict[str, float]:
    """Compute simple metrics between two elapsed-time series (seconds).
    ...
    if t.size != a.size or t.size != b.size:
        raise ValueError("time, a, b must have equal length")
    diff = b - a
    mean_offset = float(np.mean(diff))
    std_offset = float(np.std(diff, ddof=1)) if diff.size > 1 else 0.0
    max_abs_error = float(np.max(np.abs(diff)))
    if t[-1] > t[0]:
        final_drift_rate = float((diff[-1] - diff[0]) / (t[-1] - t[0]))
    else:
        final_drift_rate = 0.0
    return {
...
def plot_comparison(timeseries_dict: Dict[str, np.ndarray], labels: List[str] | None = None) -> None:
    """Matplotlib plot of elapsed time for each clock and their pairwise differences."""
    ...
    # difference first two if present
    if len(keys) >= 2:
